...
object Mst5MediaClient {
    ...
    @JvmStatic @Throws(Exception::class) fun uploadE2EDescriptorWithKey(endpoint: String?, publicKeyB64: String?, ticket: String?, fileId: String?, plaintextSize: Long, source: ParcelFileDescriptor?, mediaKey: ByteArray?, fileAad: ByteArray?, observer: Observer?) = NativeMst5.uploadE2EWithKey(endpoint, publicKeyB64, ticket, fileId, plaintextSize, source, mediaKey, fileAad, adapt(observer))
    ...
    @JvmStatic @Throws(Exception::class) fun downloadE2EDescriptorWithKey(endpoint: String?, publicKeyB64: String?, ticket: String?, fileId: String?, encryptedSize: Long, target: ParcelFileDescriptor?, mediaKey: ByteArray?, fileAad: ByteArray?, observer: Observer?): Long = NativeMst5.downloadE2EWithKey(endpoint, publicKeyB64, ticket, fileId, encryptedSize, target, mediaKey, fileAad, adapt(observer))
    ...
    @JvmStatic @Throws(Exception::class) fun downloadE2EBytesWithKey(endpoint: String?, publicKeyB64: String?, ticket: String?, fileId: String?, encryptedSize: Long, maxBytes: Int, mediaKey: ByteArray?, fileAad: ByteArray?): ByteArray = NativeMst5.downloadE2EBytesWithKey(endpoint, publicKeyB64, ticket, fileId, encryptedSize, maxBytes, mediaKey, fileAad)
